...
{
    mother->sprite.pos.x = mother->sprite.elem[i].x - mother->gps.pos.x;
    mother->sprite.pos.y = mother->sprite.elem[i].y - mother->gps.pos.y;
    mother->sprite.inv_det = 1.0 / (mother->gps.plane.x * mother->gps.dir.y - mother->gps.dir.x * mother->gps.dir.y);
...
static void    sprite_display(t_general *mother, int j, int stripe)
{
    ...
    if ((mother->mlx.img_ray.color & 0x00FFFFFF) != 0) 
        draw_pixel(&(mother->mlx.img_ray), stripe, txtr_y);
}
...
void    sprite_projection(t_general *mother)
{
    ...
    while (i < mother->sprite.sprite_count)
    {
        ...
        {
            mother->sprite.tex.x = (int)(256 * (stripe - (-mother->sprite.sprite_width / 2 + mother->sprite.spriteScreen)) * mother->args.text[4].text_width / mother->args.text[4].text_width) / 256;
